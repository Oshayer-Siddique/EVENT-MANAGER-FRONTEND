...
const SeatMapDesignerPage = ({ params }: SeatMapPageProps) => {
          ...
          <section className="space-y-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <header className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex items-center gap-2 text-sm text-slate-500">
                ...
                <span className="inline-flex items-center gap-1">
                  <span className="h-3 w-3 rounded-full bg-[#2F5F7F]" /> Assigned
                </span>
              </div>
            </header>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        onClick={() => {
                          setSelectedSeatIds(prev => {
                            const next = new Set(prev);
                            const allSelected = seats.every(seat => next.has(seat.seatId));
                            seats.forEach(seat => {
                              if (seat.status !== EventSeatStatus.AVAILABLE) {
                                return;
                              }
...
const SeatButton = ({ seat, color, selected, onClick }: SeatButtonProps) => {
  const disabled = seat.status !== EventSeatStatus.AVAILABLE;
  const background = color ? withAlpha(color, selected ? 0.35 : 0.18) : selected ? "#cbd5f5" : "#f1f5f9";
  const borderColor = color ? withAlpha(color, 0.8) : "#cbd5f5";
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      title={`${displayLabel} · ${seat.tierCode ?? "Unassigned"}`}
      className={cn(
        "flex h-12 w-12 flex-col items-center justify-center rounded-xl border text-xs font-semibold transition",
        disabled ? "cursor-not-allowed bg-slate-200 text-slate-400" : "hover:translate-y-[-1px] hover:shadow",
        selected && !disabled ? "ring-2 ring-offset-2 ring-[#2F5F7F]" : ""
      )}
      style={{ backgroundColor: disabled ? undefined : background, borderColor: borderColor }}
    >
      <span className="leading-tight text-slate-800">{displayLabel}</span>
